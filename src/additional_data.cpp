...
void AdditionalCacheData::OnAccess(const request_t* req) {
    ...
    if (!object_in_cache_metadata) {
        object_in_cache_metadata = std::make_unique<ObjectInCacheMetadata>();
    }
    ...
    if (object_lifetime_metadatas) {
        ...
        if (object_offline_clock_metadatas) [[likely]] {
            // currently_none
        }
        if (object_offline_clock_v2_metadatas) [[likely]] {
    ...
    if (object_extra_metadatas) {
        ...
        if (!object_extra_metadata) {
            object_extra_metadata = std::make_unique<ObjectExtraMetadata>();
        }
...
void AdditionalCacheData::OnPromotion(const cache_obj_t* obj_promoted, const request_t* req) {
    ...
    auto& object_in_cache_metadata = object_in_cache_metadatas[obj_promoted->obj_id];
    if (!object_in_cache_metadata) [[unlikely]] {
        throw std::runtime_error("Somehow, object_in_cache_metadata is not initialized");
    }
    object_in_cache_metadata->cache_freq = 0;
    if (object_extra_metadatas) {
        auto& object_extra_metadata = object_extra_metadatas.value()[obj_promoted->obj_id];
        if (!object_extra_metadata) [[unlikely]] {
            throw std::runtime_error("Somehow, object_extra_metadata is not initialized");
        }
