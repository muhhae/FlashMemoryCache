...
class QORClockData {
   public:
    ...
    std::unordered_map<obj_id_t, QORClockMetadata> metadatas;
...
void OnInsert(data::AdditionalCacheData& data, const request_t* req) {
    auto* cache_data = std::any_cast<QORClockData>(&data.CacheSpecificData);
    assert(cache_data);
    cache_data->metadatas.emplace(req->obj_id, QORClockMetadata());
}
...
void SetParams(cache_t* cache, std::unordered_map<std::string, std::string>& params) {
    ...
    data.CacheSpecificData.emplace<QORClock::QORClockData>(p);
}
...
void QORClockEvict(cache_t* cache, const request_t* req) {
...
cache_t* QORClockInit(
    const common_cache_params_t ccache_params, const char* cache_specific_params
) {
    auto cache = Clock_init(ccache_params, cache_specific_params);
    cache->cache_init = QORClockInit;
    cache->evict = QORClock::QORClockEvict;

    auto& data = data::AdditionalCacheDataStorage::GetStorage().GetAdditionalCacheData(cache);

    data.OnAccessCallback = QORClock::OnAccess;
    data.OnEvictionCallback = QORClock::OnEviction;
    data.OnInsertCallback = QORClock::OnInsert;
    data.OnIterationEndCallback = QORClock::OnIterationEnd;
    data.SetParamsCallback = QORClock::SetParams;
