   ...
   public:
    ...
    void OnEviction(const cache_obj_t* obj_evicted, const request_t* req) {
        ...
        if (metadata.reinserted) {
            freq_threshold += step * ((freq_threshold <= UINT64_MAX - step) &&
                                      (metadata.freq_reinserted > freq_threshold));
            time_threshold -= step * ((time_threshold >= step) &&
...
void OnInsert(data::AdditionalCacheData& data, const request_t* req) {
    auto* cache_data = std::any_cast<TAutoData>(&data.CacheSpecificData);
    assert(cache_data);
    cache_data->metadatas.emplace(req->obj_id, TAutoMetadata());
...
void SetParams(cache_t* cache, std::unordered_map<std::string, std::string>& params) {
    ...
    data.CacheSpecificData.emplace<TAuto::TAutoData>(ghost_q_size * cache_size, step);
}

void TAutoEvict(cache_t* cache, const request_t* req) {
    auto& additional_cache_data = data::AdditionalCacheDataStorage::GetStorage()
                                      .GetAdditionalCacheData(cache);
    Clock_params_t* params = (Clock_params_t*)cache->eviction_params;
    cache_obj_t* obj_to_evict = params->q_tail;
    auto* cache_data = std::any_cast<TAutoData>(&additional_cache_data.CacheSpecificData);
...
cache_t* TAutoInit(const common_cache_params_t ccache_params, const char* cache_specific_params) {
    ...
    cache->cache_init = TAutoInit;
    cache->evict = TAuto::TAutoEvict;

    auto& data = data::AdditionalCacheDataStorage::GetStorage().GetAdditionalCacheData(cache);

    data.OnAccessCallback = TAuto::OnAccess;
    data.OnEvictionCallback = TAuto::OnEviction;
    data.OnInsertCallback = TAuto::OnInsert;
    data.OnIterationEndCallback = TAuto::OnIterationEnd;
    data.SetParamsCallback = TAuto::SetParams;
