...
namespace algorithm {
namespace QTimeClock {

struct QTimeClockMetadata {
    uint64_t last_access_time = 0;
};
class QTimeClockData {
   public:
    QTimeClockData(float p = 0.1) : quantile(1 - p) {
    }
    uint64_t GetQuantile(uint64_t new_time) {
        quantile.add(new_time);
        return quantile.get();
    }
    std::unordered_map<obj_id_t, QTimeClockMetadata> metadatas;

   private:
    P2Quantile quantile;
};
void OnInsert(data::AdditionalCacheData& data, const request_t* req) {
    auto* cache_data = std::any_cast<QTimeClockData>(&data.CacheSpecificData);
    assert(cache_data);
    cache_data->metadatas.emplace(req->obj_id, QTimeClockMetadata());
}
...
void SetParams(cache_t* cache, std::unordered_map<std::string, std::string>& params) {
    ...
    data.CacheSpecificData.emplace<QTimeClock::QTimeClockData>(p);
}
...
void QTimeClockEvict(cache_t* cache, const request_t* req) {
...
cache_t* QTimeClockInit(
    const common_cache_params_t ccache_params, const char* cache_specific_params
) {
    auto cache = Clock_init(ccache_params, cache_specific_params);
    cache->cache_init = QTimeClockInit;
    cache->evict = QTimeClock::QTimeClockEvict;

    auto& data = data::AdditionalCacheDataStorage::GetStorage().GetAdditionalCacheData(cache);

    data.OnAccessCallback = QTimeClock::OnAccess;
    data.OnEvictionCallback = QTimeClock::OnEviction;
    data.OnInsertCallback = QTimeClock::OnInsert;
    data.OnIterationEndCallback = QTimeClock::OnIterationEnd;
    data.SetParamsCallback = QTimeClock::SetParams;
